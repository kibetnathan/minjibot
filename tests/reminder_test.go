...
func TestParseDuration(t *testing.T) {
	...
	for _, tc := range cases {
		if got := commands.ParseDuration(tc.in); got != tc.want {
			t.Errorf("commands.ParseDuration(%q) = %v, want %v", tc.in, got, tc.want)
		}
	}
}
...
func TestIsClock(t *testing.T) {
	for _, s := range []string{"14:00", "09:30", "23:59", "00:00"} {
		if !commands.IsClock(s) {
			t.Errorf("commands.IsClock(%q) = false, want true", s)
		}
	}
	for _, s := range []string{"9:00", "14:0", "1a:00", "12:3a", "12345", "", "14:00:00"} {
		if commands.IsClock(s) {
			t.Errorf("commands.IsClock(%q) = true, want false", s)
		}
	}
}
...
func TestHumanizeDuration(t *testing.T) {
	...
	for _, tc := range cases {
		if got := commands.HumanizeDuration(tc.in); got != tc.want {
			t.Errorf("commands.HumanizeDuration(%v) = %q, want %q", tc.in, got, tc.want)
		}
	}
}
...
func TestParseReminderArgs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			text, d := commands.ParseReminderArgs(tc.args)
...
func TestParseReminderArgsClock(t *testing.T) {
	...
	text, d := commands.ParseReminderArgs([]string{"in", "14:00", "lunch"})
	if text != "lunch" {
