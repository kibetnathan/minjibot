...
func TestMemegenText(t *testing.T) {
	...
	for _, tc := range cases {
		if got := commands.MemeGenText(tc.in); got != tc.want {
			t.Errorf("commands.MemeGenText(%q) = %q, want %q", tc.in, got, tc.want)
		}
	}
}
...
func TestParseCaptionArgs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			top, bottom, image := commands.ParseCaptionArgs(tc.args)
