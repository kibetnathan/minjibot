...
func TestSnowflakeFromValue(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := commands.SnowflakeFromValue(tc.in); got != tc.want {
				t.Errorf("commands.SnowflakeFromValue(%v) = %q, want %q", tc.in, got, tc.want)
			}
		})
	}
}
...
func TestBuildPinglistEmbedMatches(t *testing.T) {
	target := &commands.PingTarget{Kind: "role", ID: "999", Name: "admins"}
...
func TestBuildPinglistEmbedTruncates(t *testing.T) {
	...
	for i := 0; i < commands.PinglistMaxResults+5; i++ {
