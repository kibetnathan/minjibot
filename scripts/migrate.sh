#!/bin/sh
# Runs database migrations with goose before the bot starts.
#
# Requires:
...
if [ -z "$GOOSE_DBSTRING" ]; then
  echo "ERROR: GOOSE_DBSTRING is not set. Provide the Postgres connection string." >&2
  exit 1
fi

echo "Running database migrations (driver=$GOOSE_DRIVER, dir=$GOOSE_MIGRATION_DIR)..."
