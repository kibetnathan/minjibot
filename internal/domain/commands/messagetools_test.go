...
func TestParseMessageLink(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			g, c, m, ok := parseMessageLink(tc.in)
			if g != tc.wantGuild || c != tc.wantChannel || m != tc.wantMessage || ok != tc.wantOK {
				t.Errorf("parseMessageLink(%q) = (%q,%q,%q,%v), want (%q,%q,%q,%v)",
...
func TestMessageToLink(t *testing.T) {
	...
	if got := messageToLink(withGuild); got != "https://discord.com/channels/g/c/m" {
		t.Errorf("got %q", got)
	}
	dm := &discordgo.Message{ChannelID: "c", ID: "m"}
	if got := messageToLink(dm); got != "https://discord.com/channels/@me/c/m" {
		t.Errorf("got %q", got)
	}
}
