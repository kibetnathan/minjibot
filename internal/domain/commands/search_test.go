...
func TestParseSearchArgs(t *testing.T) {
	cases := []struct {
		...
		{"simple", []string{"hello", "world"}, "hello world", searchDefaultLimit, false},
		{"trailing number", []string{"hello", "50"}, "hello", 50, false},
		{"messages flag", []string{"messages:75", "hello"}, "hello", 75, false},
		{"clamp low", []string{"hello", "0"}, "hello", searchDefaultLimit, false},
		{"clamp high", []string{"messages:9999", "hi"}, "hi", searchDefaultLimit, false},
		{"bad messages flag", []string{"messages:abc", "hi"}, "hi", searchDefaultLimit, false},
		{"bare number only", []string{"50"}, "", 50, false},
		{"number mid query stays", []string{"version", "3", "released"}, "version released", 3, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			q, n, err := parseSearchArgs(&discordgo.Session{}, "", tc.args)
...
func TestBuildSearchEmbedNoMatches(t *testing.T) {
	embed := buildSearchEmbed("xyzzy", 100, nil)
...
func TestBuildSearchEmbedMatches(t *testing.T) {
	long := strings.Repeat("x", searchMaxFieldLength+100)
	...
	embed := buildSearchEmbed("found", 200, msgs)
	...
	if len(contentPart) > searchMaxFieldLength || !strings.HasSuffix(contentPart, "...") {
		t.Errorf("content not truncated to %d: len=%d value=%q", searchMaxFieldLength, len(contentPart), contentPart[:min(len(contentPart), 80)])
	}
...
func TestBuildSearchEmbedTruncatesResults(t *testing.T) {
	...
	for i := 0; i < searchMaxResults+2; i++ {
	...
	embed := buildSearchEmbed("hit", 200, msgs)
	if len(embed.Fields) != searchMaxResults {
		t.Errorf("len(fields) = %d, want %d", len(embed.Fields), searchMaxResults)
	}
